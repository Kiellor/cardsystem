...
class CharactersController extends AppController {
	...
	public function view($cardnumber = null, $collapse = false, $page = -1) {
		...
		$lastEdited = $db->fetchAll('SELECT p.name, u.username, cel.logged_at from character_edit_log cel LEFT OUTER JOIN users u on u.id = cel.user_id LEFT OUTER JOIN players p on p.id = u.player_id where cel.character_id = ? order by cel.logged_at desc',array($id));

		if(count($lastEdited) > 0) {
			$this->set('lastEditedBy',$lastEdited[0]);
		}
		...
		$this->set('build_earned_from_death',$build_earned_from_death);
		...
		if($page == -1 || $page == $character['Character']['itempage']) {
			...
			// Set the Card to Only Read from the Pre 2.0 Abilities Table
			$results = $db->fetchAll(
				'SELECT sum(ia.qty) as qty, a.display_name, a.id, a.ability_name, a.abilitytype_id, a.abilitygroup_id, ia.slot_number, ia.socket_number from item_abilities ia LEFT OUTER JOIN abilities_pre2 a on ia.ability_id = a.id where ia.character_id = ? group by ia.slot_number, a.id, a.display_name, a.ability_name, ia.socket_number order by ia.slot_number, ia.socket_number',
				array($id)
			);
			foreach($results as $row) {
				...
				} else if($row['a']['abilitygroup_id'] != 56) {
					$i = $row[0]['qty'];
					while($i > 5) {
						if(strlen($bubbles) > 0) {
							$bubbles .= "\n";
    ...
    public function loadPlayers() {
    	$results = $this->Player->find('all',array(
    		'fields' => array('Player.id','Player.name','Player.cardnumber_prefix','Player.active'),
    ...
    public function getRenumber() {
    	...
    	$results = $db->fetchAll('SELECT p.id, p.name, p.cardnumber_prefix, p.active, c.id, c.cardnumber, c.name from players p left outer join characters c on c.player_id = p.id where c.cardnumber not like CONCAT(p.cardnumber_prefix, "%") order by p.cardnumber_prefix, c.cardnumber');
