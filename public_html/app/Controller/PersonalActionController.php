...
class PersonalActionController extends AppController {
	...
	public function warstats() {
		$db = $this->War->getDataSource();

		$results = $db->query('SELECT sum(run) as run, sum(hide) as hide, sum(fight) as fight, count(*) as total from wars');

		$this->set('ajax',json_encode($results[0][0]));
		$this->layout = 'ajax';
		$this->render('ajax');
	}
	...
	public function saveVotes() {
		...
		if($data['War']['id'] == null) {
			...
			$this->War->set(array(
				...
				'run' => $data['War']['run'],
				'hide' => $data['War']['hide'],
				'fight' => $data['War']['fight']
			));
			$this->War->save();
		} else {
			$this->War->id = $data['War']['id'];
			$this->War->set(array(
				'run' => $data['War']['run'],
				'hide' => $data['War']['hide'],
				'fight' => $data['War']['fight']
	...
	public function loadDefaultActions() {
		...
		$db->query('insert into turn_actions (land_id, settlement_id, business_id, improvement_id, action_id, turn_id, character_id, target, comments, is_default, action_number) select land_id, settlement_id, business_id, improvement_id, action_id, ? as turn_id, character_id, target, comments, 1 as is_default, action_number from default_actions where character_id not in (select character_id from turn_actions where turn_id = ?) and character_id in (select id from characters where cset_id = 1 and active = 1)',array($current_turn_id,$current_turn_id));

		$db->query('insert into turn_actions (land_id, settlement_id, business_id, improvement_id, action_id, turn_id, character_id, target, comments, is_default, action_number) select land_id, settlement_id, business_id, id as improvement_id, 27 as action_id, ? as turn_id, 0 as character_id, "" as target, "no action" as comments, 1 as is_default, 0 as action_number from settlement_improvements WHERE business_id > 0 and id not in (select improvement_id from turn_actions where turn_id = ?)',array($current_turn_id,$current_turn_id));
	...
	public function loadlands() {
		$results = $this->Land->find('all',array('conditions' => array('Land.active' => 1)));
	...
	private function getPrevTurn() {
		$db = $this->TurnAction->getDataSource();
		$results = $db->fetchAll('SELECT t.id, t.status, t.started, t.ended, t.defaults_loaded from turns t where id in (SELECT max(id) from turns where id < (select max(id) from turns))');
	...
	public function finalizeturn() {
		$results = $this->getPrevTurn();

		$this->Turn->id = $results['t']['id'];
		$this->Turn->saveField('status',3);
	...
	public function returnToEndOfTurn() {
		$results = $this->getPrevTurn();
	...
	public function saveactionresults2() {
		...
		$this->TurnAction->set(array(
			...
			'marshal_id'		=> AuthComponent::user('id')
		));
		...
		if($data['TurnAction']['commodity_value'] != 0) {
			$this->BusinessDeposit->create();
			switch($data['TurnAction']['commodity']) {
				case 'Luxuries':
					$this->BusinessDeposit->set(array('luxury' => $data['TurnAction']['commodity_value']));
					break; 
				case 'Durables':
					$this->BusinessDeposit->set(array('durable' => $data['TurnAction']['commodity_value']));
					break;
				case 'Wearables':
					$this->BusinessDeposit->set(array('wearable' => $data['TurnAction']['commodity_value']));
					break;
				case 'Consumables':
					$this->BusinessDeposit->set(array('consumable' => $data['TurnAction']['commodity_value']));
					break;
			}
			$this->BusinessDeposit->set(array(
				'business_id' 		=> $data['TurnAction']['business_id'],
 ...
 //  		}
	...
	public function loadhistory($settlement_id) {
		...
		$turnids = [];
		$turnids[0] = $turns[1]['Turn']['id'];
		$turnids[1] = $turns[2]['Turn']['id'];
		$turnids[2] = $turns[3]['Turn']['id'];

		$results = $this->TurnAction->find('all',array(
			'conditions' => array('Settlement.id' => $settlement_id, 'Turn.id' => $turnids),
			'order' => array('Turn.id desc', 'Character.name'),
			'contain' => array(
	...
	public function getTradesForCharacter($character_id) {
		$db = $this->Character->getDataSource();

		$results = $db->fetchAll(
			'Select sum(quantity) as qty, a.id, a.display_name as display, a.ability_name as ability, ao.ability_name as opt, ao.display_name as opt_disp from character_abilities ca LEFT OUTER JOIN abilities a on ca.ability_id = a.id LEFT OUTER JOIN abilities ao on ca.abilityoption_id = ao.id where ca.character_id = ? and a.id in (26094,26095,26096,26097,26098,26099,26100,26101) group by a.id, ao.id', array($character_id));

		$trades = [];
		$trades['Luxuries']  = 0;
		$trades['Durables'] = 0;
		$trades['Consumables'] = 0;
		$trades['Wearables'] = 0 ;

		foreach($results as $row) {
			if($row[0]['qty'] > 0) {
				switch ($row['a']['ability']) {
				case 'Trade Durable':
					break;
				case 'Trade Wearable':
					break;
				case 'Trade Consumable':
					break;
				case 'Trade Luxury':
					break;
				case 'Trade Durable Proficiency +1':
					$trades['Durables'] += 3 * $row[0]['qty'];
					break;
				case 'Trade Wearable Proficiency +1':
					$trades['Wearables'] += 3 * $row[0]['qty'];
					break;
				case 'Trade Consumable Proficiency +1':
					$trades['Consumables'] += 3 * $row[0]['qty'];
					break;
				case 'Trade Luxury Proficiency +1':
					$trades['Luxuries'] += 3 * $row[0]['qty'];
					break;
				}
			}
		}
	...
	public function saveimprovement() {
		$data = $this->request->input('json_decode',true);
  		...
  		$this->SettlementImprovement->set(array(
  			...
  			'actions_to_repair' =>  $data['actions_to_repair']
  		));
  		$this->SettlementImprovement->save();
