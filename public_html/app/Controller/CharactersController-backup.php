...
class CharactersController extends AppController {
    ...
    public function newbodyajax($cardnumber = null) {
		...
		$results = $db->fetchAll(
			'SELECT sum(ca.quantity) as total FROM character_abilities ca LEFT OUTER JOIN abilities a on a.id = ca.ability_id WHERE a.ability_name = "Psionicist" and ca.character_id = ?',
			array($id)
		);
	...
	public function newbody($cardnumber = null) {
		...
		$ratioresults = $db->fetchAll(
			'SELECT sum(ca.quantity * ca.build_spent * a.ratio) / sum(ca.quantity * ca.build_spent) as total FROM character_abilities ca LEFT OUTER JOIN abilities a on a.id = ca.ability_id WHERE a.ratio > 0 and ca.character_id = ?',
			array($id)
		);
	...
	public function view($cardnumber = null, $collapse = false, $page = -1) {
		...
		$this->filename = sprintf("%04d_%s",$cardnumber,str_replace(" ","_",$character['Character']['name']));
	...
	public function downloadpdf_aux($cardnumber = null, $page, $collapse = true ) {
		...
		$this->set('filename',$this->filename."_".$page);
		$this->set('fpdf', new FPDF('P','mm','Letter'));

		$this->render('downloadpdf_aux');
	}

    public function add($playername = null) {
