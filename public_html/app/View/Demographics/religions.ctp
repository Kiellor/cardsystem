...
<script type="text/javascript">
	...
	var data_in = [<?php
		$religious_folks;
		foreach($religion as $datum) { // First, count all the fanatics.
			$religious_folks = $religious_folks + $datum[0]['count'];
		}
		
		//now use that to spit out the number of godless heathens.
		//(This assumes that secularism is the predominant "religion" in Travance.)
		$total = 0;
		
		echo '{"label":"'.'Secular (Unbaptised)'.'","value":'.($guff[0][0]['count']-$religious_folks).',"start":'.$total.',"end":'.($guff[0][0]['count']-$religious_folks).'},';
		
		$total = ($guff[0][0]['count']-$religious_folks);
		foreach($religion as $datum) { //now spit out the rest of the array.
			...
			$total = $total + $count;
			$religious_folks = $religious_folks + $count;
		}
