...
<div id="title">Number of (lower) lists<br></div>
<div id="graph"></div>
...
<script type="text/javascript">
	...
	var data_in = [<?php
		...
		foreach($listdata as $datum) {
			$racename = $datum['not_as_small']['num_lists'];
			$count = $datum[0]['num_playaz'];
	...
	//A 1-row table, to organise things on the page.
	...
	var my_svg = table.append("td")
		    .append("svg")
		    .attr("id", "svg-frame")
	//Set the width and height of this frame (Geoff, this ones for you!)
		    .attr("width", (2*pie_r_outer))
		    .attr("height", (2*pie_r_outer))
		    .data(data_in);		     
	...
	//radio buttons.
	var radio = table.append("td")
			.append("form")
			.selectAll("#radio_options")
		    	.data(data_in)
		    	.enter()
		    	.append("input")
		    	.attr("id", function(d, i) { return "radio_"+i; } )
		    	.attr("class", "pie_arc");
