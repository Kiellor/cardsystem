...
<div ng-app="myApp">
	...
	<script type="text/javascript">
		...
		myApp.controller('NPMController',['$scope','$http','$timeout','FireObject','FireArray', function($scope, $http, $timeout, FireObject, FireArray) {
			...
			$scope.saveAlias = function() {
				...
				var aliasRef = new Firebase('https://shining-torch-9307.firebaseio.com/presence/users/' + $scope.userid + '/alias');
			...
			$scope.forgetToken = function(token) {
				var tokenRef = new Firebase('https://shining-torch-9307.firebaseio.com/presence/users/' + $scope.userid + '/tokens/' + token.ref);
				tokenRef.remove();
			}

			$scope.rememberToken = function() {
				var tokenRef = new Firebase('https://shining-torch-9307.firebaseio.com/presence/users/' + $scope.userid + '/tokens');
			...
			$scope.getUserInformation = function() {
				...
				var amOnline = new Firebase('https://shining-torch-9307.firebaseio.com/.info/connected');
				var locationRef = new Firebase('https://shining-torch-9307.firebaseio.com/locations/' + here);
			...
			$scope.initialize = function() {
				...
				$scope.chatUsers = FireObject(["locations","character_builder-"+$scope.token]);
	...
	<div ng-controller="NPMController" ng-init="initialize()">
		...
		<div ng-show="meta && !meta.stage">
			...
			No character with token {{token}} exists yet 
